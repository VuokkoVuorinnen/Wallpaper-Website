function WallpaperApi() {
    var api = this;
    this.apiUrl = "https://wallpapers.carroarmato0.be/v3/api/";
    this.format = "json";
    ...
    this.GetTags = function (callback) {
        _onLoading();
        $.getJSON(this.apiUrl + '?request=getTags&format=' + this.format, function (data) {
            if (callback) callback(data);
            _onLoaded();
        });
    }
    this.GetTagCloud = function (callback) {
        _onLoading();
        $.getJSON(this.apiUrl + '?request=getTagCloud&format=' + this.format, function (data) {
            if (callback) callback(data);
            _onLoaded();
        });
    }
